...
# Datos de fecundidad huterita para usar como referencia
asfrs_ht <- read.csv(file.path("datos", "asfrs_ht.csv"), header = TRUE)
...
su <- 1.6 # desviación estandar de la distribución de edad a la union
...
ns <- 11 # período de no suceptibilidad
fi <- 0.2
...
# genere todas las trayectorias reproductivas de una chorte de n mujeres,
...
# n mayor
ls_hst <- gen_hst(n = 1000, fi = 0.2, ns = 11, mu = 20, su = 1.6)

# plot f(x)
plot_fx(ls_hst[[length(ls_hst)]])

hutterite_col <- grDevices::adjustcolor("purple", alpha.f = 0.65)

points(asfrs_ht$age[asfrs_ht$age %in% 10:49],
       asfrs_ht$fx[asfrs_ht$age %in% 10:49],
       pch = 19,
       col = hutterite_col)

legend("topleft",
       legend = c("Modelo simple", "Huteritas"),
       pch = c(1, 19),
       col = c("black", hutterite_col),
       bty = "n")
