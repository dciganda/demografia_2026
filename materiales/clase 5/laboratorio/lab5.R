...
# 5to Laboratorio: Modelos del proceso reproductivo                            # 
...
# población de referencia tomando el tamañano de la cohorte = 2000.
...
plot_sum_fx <- function(dat, lines = FALSE, ...) {
  n <- attr(dat, "n_cohort", exact = TRUE)
  fx <- table(factor(floor(dat$edad), levels = 10:50)) / n
  if (lines) {
    lines(10:50, fx, ...)
  } else {
    plot(10:50, fx, type = "l",
         xlab = "Edad (años)", ylab = "Tasa específica de fecundidad", ...)
  }
  sum(fx)  # TFR
}
...
#################################################
# Coeficiente beta1: edades jóvenes             #
#################################################
beta1_vals <- seq(0.32, 0.72, 0.04)
...
plot(beta1_vals, tfr_dif_beta1)

# Describa lo que se observa en el gráfico


#################################################
# Coeficiente beta2: edades avanzadas           #
#################################################
beta2_vals <- seq(-0.12, 0.04, 0.01)
